...
func _process(delta: float) -> void:
	...
	var camera_aim = $Camera3D.get_global_transform().basis
	...
	$Camera3D.transform.origin.y = sin(deg_to_rad(camera_angle)) * camera_distance
	$Camera3D.transform.origin.z = cos(deg_to_rad(camera_angle)) * camera_distance
	
	if Input.is_action_pressed("player_angle"):
		camera_angle += mouse_delta_position.y * 0.25
		camera_angle = clamp(camera_angle, 35.0, 85.0)
		$Camera3D.transform = $Camera3D.transform.looking_at(Vector3(0.0, 0.0, 0.0), Vector3(0.0, 1.0, 0.0))
